...
use jpc::{
    decode_j2c, CodingStyle, CodingStyleMarkerSegment, ContiguousCodestream, Header,
    ImageAndTileSizeMarkerSegment,
};
use std::error;
use std::fmt;
use std::fs::File;
...
fn encode_cod<W: io::Write>(
    ...
    write!(
        ...
        segment.no_decomposition_levels()
    )?;
    write!(
        writer,
        "        <xjp:xcb>{}</xjp:xcb>\n",
        segment.code_block_width()
    )?;
    write!(
        writer,
        "        <xjp:ycb>{}</xjp:ycb>\n",
        segment.code_block_height()
    )?;
    write!(
        writer,
        "        <xjp:style>{}</yjp:style>\n",
        segment.code_block_style()
    )?;
    write!(
        writer,
        ...
        .contains(&CodingStyle::EntropyCoderWithPrecinctsDefined)
    {
        write!(
            writer,
            "        <xjp:ppx>{}</xjp:ppx>\n",
            segment.precinct_width_exponent()
        )?;
        write!(
            writer,
            "        <xjp:ppy>{}</xjp:ppy>\n",
            segment.precinct_height_expontent()
        )?;
    }
    write!(writer, "      </xjp:SPcod>\n",)?;
...
fn encode_contiguous_codestream<W: io::Write>(
    ...
    contiguous_codestream_box: &ContiguousCodestreamBox,
    contiguous_codestream: &ContiguousCodestream,
) -> Result<(), Box<dyn error::Error>> {
    write!(
        writer,
        "  <xjp:jp2c type=\"box\" length=\"{}\" offset=\"{}\">\n",
        contiguous_codestream_box.length(),
        contiguous_codestream_box.offset()
    )?;
...
pub fn encode_jpxml<W: io::Write>(
    ...
    for contiguous_codestream_box in jp2.contiguous_codestreams_boxes() {
        ...
        let contiguous_codestream = decode_j2c(&mut reader)?;
