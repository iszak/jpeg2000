...
// jp2\040
const BRAND: [u8; 4] = [106, 112, 50, 32];
...
impl JBox for HeaderBox {
    ...
    fn decode<R: io::Read + io::Seek>(
        ...
        info!("ImageHeaderBox Start {:?}", self.image_header_box.offset);
        self.image_header_box.decode(reader)?;
        info!("ImageHeaderBox Finish {:?}", reader.stream_position());
        ...
        loop {
            ...
            match BoxTypes::new(box_type) {
                ...
                BoxTypes::ComponentMapping => {
                    ...
                    info!(
                        "ComponentMappingBox Start {:?}",
                        component_mapping_box.offset
                    );
                    component_mapping_box.decode(reader)?;
                    info!("ComponentMappingBox Finish {:?}", reader.stream_position());
                ...
                BoxTypes::ChannelDefinition => {
                    ...
                    info!(
                        "ChannelDefinitionBox Start {:?}",
                        channel_definition_box.offset
                    );
                    channel_definition_box.decode(reader)?;
                    info!("ChannelDefinitionBox Finish {:?}", reader.stream_position());
                ...
                BoxTypes::Resolution => {
                    ...
                    info!("ResolutionBox Start {:?}", resolution_box.offset);
                    resolution_box.decode(reader)?;
                    info!("ResolutionBox Finish {:?}", reader.stream_position());
...
impl ImageHeaderBox {
    ...
    pub fn intellectual_property(&self) -> bool {
        self.intellectual_property[0] == 1
    }
}
        ...
        loop {
            ...
            match BoxTypes::new(box_type) {
                BoxTypes::CaptureResolution => {
                    ...
                    info!(
                        "CaptureResolutionBox Start {:?}",
                        capture_resolution_box.offset
                    );
                    capture_resolution_box.decode(reader)?;
                    info!("CaptureResolutionBox Finish {:?}", reader.stream_position());
                ...
                BoxTypes::DefaultDisplayResolution => {
                    ...
                    info!(
                        "DisplayResolutionBox Start {:?}",
                        default_display_resolution_box.offset
                    );
                    default_display_resolution_box.decode(reader)?;
                    info!("DisplayResolutionBox Finish {:?}", reader.stream_position());
...
pub fn decode_jp2<R: io::Read + io::Seek>(
    ...
    info!("SignatureBox Start {:?}", signature_box.length);
    signature_box.decode(reader)?;
    info!("SignatureBox Finish {:?}", reader.stream_position());
    ...
    info!("FileTypeBox Start {:?}", file_type_box.offset);
    file_type_box.decode(reader)?;
    info!("FileTypeBox Finish {:?}", reader.stream_position());
    ...
    loop {
        ...
        match BoxTypes::new(box_type) {
            BoxTypes::Header => {
                ...
                info!("HeaderBox Start {:?}", reader.stream_position());
                ...
                info!("HeaderBox Finish {:?}", reader.stream_position());
            }
